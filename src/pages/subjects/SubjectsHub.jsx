...
const GROUPS = [
  {
    key: "scientific",
    title: "بابەتە زانستییەکان",
    items: ["بیرکاری", "کیمیا", "فیزیا", "زیندەزانی"],
  },
  {
    key: "languages",
    title: "زمانەکان",
    items: ["ئینگلیزی", "کوردی", "عەرەبی", "ئیسلام"],
  },
  {
    key: "literary",
    title: "بابەتە ئەدەبییەکان",
    items: ["مێژوو", "ئابووری", "جووگرافیا", "بیرکاری وێژەیی"],
  },
];

const streamFromGroupKey = (k) => (k === "scientific" ? "scientific" : k === "literary" ? "literary" : "both");
...
function PageHeader({ q, setQ, suggestions, onPick }) {
  ...
  useEffect(() => {
    const h = (e) => { if (!boxRef.current) return; if (!boxRef.current.contains(e.target)) setOpen(false); };
    ...
    <div dir="rtl" className="relative">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-500/10 to-sky-500/5 p-3 sm:p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="hidden sm:block text-[12px] text-zinc-300">بەخێربێیت — دەرچووی کۆتایی بۆ داهاتووەکەت</div>
        </div>
          ...
          <AnimatePresence>
            ...
            {open && suggestions.length > 0 && (
              <motion.div
                ...
                className="absolute z-50 mt-2 right-0 w-full"
              >
...
function SubjectCard({ name, onClick }) {
  return (
    <motion.button
      ...
      transition={SPRING}
      onClick={onClick}
      className="group relative rounded-3xl bg-zinc-900/60 overflow-hidden ring-1 ring-white/15 hover:ring-2 hover:ring-sky-400/40 transition-shadow shadow-[0_8px_30px_rgba(0,0,0,0.25)] text-right"
    >
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-br from-sky-400/10 to-cyan-400/10" />
...
export default function SubjectsHub() {
  ...
  return (
    <div dir="rtl" className="p-3 sm:p-5 space-y-4">
      <PageHeader q={q} setQ={setQ} suggestions={suggestions} onPick={(n) => openSubject(n, null)} />
      {shown.map((group) => (
